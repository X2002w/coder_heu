...
Coefficients (with 95% confidence bounds):
        ...
        */
        ...
        part_speed= 0.002243 *straight_dis*straight_dis+ 24.61;
        ...
        //出界，斑马线速度
        else if ((Island_State == 0 && ramp_flag==0 && zebra_line_flag)|| chujie_flag == 1)
        {
            jisha_flag=1;
            target_speed1 = 0;
        }
        ...
        else
        {
            jiasu_flag=0;
            jisha_flag=0;
            target_speed1=target_speed-part_speed;//弯道预减速
        }
        //刹车标志
        if(straight_dis<150&&hightest<45&&center_speed>target_speed &&abs(err)>45&& straight_flag==0&&Island_State==0&&ramp_flag==0)
        {
            //弯道急刹
            target_speed1=target_speed;
            jisha_flag=1;
...
void speed_contral(void)
{
   ...
   if (duty > 0) {
        ...
        if (abs(duty_ratio * duty) < 35)//右加速限幅，防止侧翻
            right_speed = target_speed1 + duty_ratio * duty;
        else
            right_speed = target_speed1 + 35;


       // left_speed = (int)(target_speed1 * (1 - (speed_ratio) * tan(5.137 * (float)duty / 4.0f * 3.14 / 1673) / 0.885));
        left_speed = target_speed1 - duty;
    }
    else {
        //右转
        if (abs(duty_ratio * duty) < 35)//左加速限幅，防止侧翻
            left_speed = target_speed1 - duty_ratio * duty;//左加速
        else
            left_speed = target_speed1 + 35;//左加速
       // right_speed = (int)(target_speed1 * (1 - (speed_ratio) * tan(5.137 * (float)(-duty) / 4.0f * 3.14 / 1673) / 0.885));
        right_speed = target_speed1 + duty;//右减速
    }
   ...
   //环岛差速
    if (Left_Island_Flag || Left_Island_Flag)
    {
        if (Island_State == 4)
        {
            duty_ratio = 0.02;//外轮是否加速
            if (duty > 0) {
                //左转
                if (abs(duty_ratio * duty) < 80)//右加速限幅，防止侧翻
                    right_speed = target_speed1 + duty_ratio * duty;
                else
                    right_speed = target_speed1 + 80;
                left_speed = target_speed1 - duty;
            }
            else {
                //右转
                if (abs(duty_ratio * duty) < 80)//左加速限幅，防止侧翻
                    ...
                    left_speed = target_speed1 + 80;//左加速
                right_speed = target_speed1 +  duty;//右减速
            }
        }
    ...
    if((Island_State == 0 && ramp_flag==0 && zebra_line_flag)|| chujie_flag == 1)
    {
        left_speed=0;
        right_speed=0;
